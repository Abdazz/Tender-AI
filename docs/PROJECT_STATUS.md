...
**Dernière mise à jour :** 2026-08-29 (session 4), par une session Claude Code — **chantier 5 (Auth & API multi-company), sous-projet A (backend) terminé et mergé.** JWT `company_id`, renommage des rôles (`admin`/`viewer` → `company_admin`/`company_viewer`), nouveau routeur `companies.py`, scoping `company_id` sur les endpoints existants, suppression des 3 stopgaps YULCOM codés en dur laissés par le chantier 3. Voir section "Chantier 5" ci-dessous pour le détail complet (5 tâches + revue finale + 1 vague de correction couvrant 10 findings, dont 2 Critical de sécurité cross-tenant). Sous-projet B (frontend `CompanyContext`) reste à faire.
...
| 5 | Auth & API multi-company | 🟡 Sous-projet A (backend) ✅ **terminé et mergé** (2026-08-29). Sous-projet B (frontend `CompanyContext`) ⬜ pas commencé | `tenderai-backend`, branche `staging` (poussé sur GitHub) |
...
**Sous-projet B — Frontend (`CompanyContext`, page `/companies`, nav "Compagnies") — ⬜ pas commencé**
- Spec : Section 4 de `docs/superpowers/specs/2026-08-23-multi-company-design.md`. Aucun plan écrit à ce jour.
- Dépend du sous-projet A (terminé) — peut démarrer quand souhaité.
