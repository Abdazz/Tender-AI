...
`Settings.app_name` (`tenderai-backend/src/tenderai_bf/config.py`) vaut encore `"TenderAI BF"` — un résidu manqué par le nettoyage doc du sous-projet A/BF-rename. Utilisé pour le titre FastAPI, le slug de nom de fichier des rapports (`reports.py`) et le nom de pièce jointe email (`smtp_client.py`) — aucune valeur exacte n'est vérifiée par les tests (`test_smoke.py` ne vérifie que la présence de l'attribut). Changé en `"TenderAI"` dans le même correctif, avec un message de commit distinct de la mécanique de renommage d'import.

### 4. `tenderai-infra/settings.yaml` — namespace de logger à synchroniser
Le sous-projet A avait explicitement laissé `logging.loggers.tenderai_bf` (clé de `settings.yaml`) inchangé « tant que le sous-projet B n'est pas fait » pour ne pas casser silencieusement le filtrage des niveaux de log. C'est maintenant le moment : cette clé, ainsi que la clé équivalente et les deux appels `structlog.get_logger("tenderai_bf")` dans `tenderai-backend/src/tenderai_bf/logging.py`, sont renommés en `tenderai` dans le même correctif — ils doivent rester synchronisés avec le nom réel du package (les loggers de module s'appellent `tenderai.xxx` après le renommage, via `get_logger(__name__)`).
