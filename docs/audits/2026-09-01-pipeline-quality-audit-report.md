...
**Constat d'ensemble : le pipeline n'a pas un problème de pertinence, il a un problème de disponibilité.** Les deux pays sont à **zéro notice en base** — la table `notices` est vide pour BF comme pour CA, toutes sources confondues. La question « le système remonte-t-il des avis non pertinents ? » est aujourd'hui structurellement sans objet : il ne remonte rien du tout, tout en continuant d'envoyer un rapport quotidien par e-mail, sans qu'aucune alerte ne signale que la collecte est à l'arrêt.

**Burkina Faso — rappel effectif : 0 %.**

- **Bout en bout : 0 avis persisté**, ni le jour de l'audit ni sur la fenêtre historique observable. Sur les 20 dernières lignes `runs` (2026-08-08 → 2026-09-01, Finding #3) : 4 runs `failed`, dont 3 sur le même `DatetimeFieldOverflow` (Finding #1, dont le run planifié de production du matin même) et 2 sur un `KeyError: 'content'` distinct jamais investigué ; les 10 runs `completed`/`completed_with_warnings` restants rapportent tous `unique_items: 0` malgré `items_parsed` entre 26 et 29 (Finding #2, anomalie non résolue). **La collecte BF est donc cassée en production depuis au moins le 2026-08-08**, soit par crash direct, soit par une perte silencieuse en amont de `deduplicate`.
...
- **Des avis manifestement pertinents IT ont été vérifiés dans les pertes**, pas seulement du volume : consommables informatiques ISLO (DGCMEF, perdu au parse), data center / réseau local CAM et licences Microsoft EA (UEMOA, perdus au fetch), pare-feu nouvelle génération EOIUNPD24643 (UNGM, perdu au fetch), acquisition et installation d'équipements informatiques PNUD/UNDP-BFA-00734 (vu par Joffres.net, perdu au persist ; également invisible côté UNGM faute de pagination).
...
| 12 | Aucune alerte quand la collecte est morte : le mail quotidien part avec un rapport vide, `counts_json`/`errors` ne signalent ni la perte à 100 % de DGCMEF, ni les 7/7 échecs OCR de Le Devoir, ni `notices_persisted: 0` | **C'est ce qui a permis à BF de rester mort ≥ 3 semaines et à CA de n'avoir jamais fonctionné, sans que personne ne le voie** | Delivery / observabilité | bug logique | **Élevée** |
...
**Récapitulatif par étiquette :** sur **28 constats**, **1 seul** porte une étiquette autre que « bug logique » côté cause racine confirmée — le point d'architecture **ouvert et non tranché** de The Commonwealth (#17). S'y ajoutent 4 réserves prospectives ou latentes, aucune confirmée comme bloquante : #18 (risque anti-bot Nova Scotia sous trafic soutenu), #20 (densité procurement faible de la source Le Devoir), #21 (champs `reference`/`deadline` absents du listing Palladium) et #26 (reCAPTCHA Commonwealth au-delà d'une profondeur de pagination). Deux constats relèvent d'incidents externes ponctuels (#27, #28). **Aucune limite technologique confirmée sur aucune des 12 sources activées.**
...
3. **La pagination : boucle `PageIndex` pour UNGM, `max_pages` + `pagination_url` pour UEMOA** (constats #7 et #6).
   Deux corrections quasi gratuites — une boucle `for` dans `fetch_ungm.py`, une mise à jour de la colonne `patterns` en base pour UEMOA (pur changement de configuration, prouvé trivial par le fait que la même mécanique tourne déjà avec succès pour Enabel) — pour **~40 avis actifs/jour côté UNGM (~74 % de sa vérité terrain) et ~184 côté UEMOA (~94,8 %)**. À elles deux, elles multiplient par ~5 le volume BF réellement fetché, et les pages jamais atteintes contiennent des marchés IT nommément vérifiés (data center / réseau local CAM, licences Microsoft EA, pare-feu nouvelle génération). Rapport impact/effort le plus élevé de tout le portefeuille BF.
