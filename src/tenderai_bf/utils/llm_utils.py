    ...
    try:
        ...
        if provider == "groq":
            if not settings.llm.groq_api_key.get_secret_value():
                logger.error(
                    "Groq API key not configured, checking fallback", fallback=fallback
                )
                if fallback:
            ...
            try:
                ...
                logger.error(
                    "langchain-groq not installed, checking fallback", fallback=fallback
                )
                if fallback:
                    logger.info("Attempting fallback to OpenAI")
                    return _get_openai_instance(temperature, max_tokens)
...
def _get_openai_instance(temperature: float, max_tokens: int) -> Any | None:
    ...
    try:
        ...
        logger.debug(
            "LLM instance created", provider="openai", model=settings.llm.openai_model
        )
        return llm
