...
async def get_recipient(recipient_id: int, db: DatabaseSession, user: AuthenticatedUser):
    from ...models import Recipient
...
async def create_recipient(request: RecipientCreate, db: DatabaseSession, user: AuthenticatedUser):
    ...
    logger.info("Recipient created", recipient_id=row.id, email=row.email, created_by=user.get("username"))
    return RecipientSchema.from_orm(row)


@router.put("/{recipient_id}", response_model=RecipientSchema)
async def update_recipient(
    recipient_id: int, request: RecipientUpdate, db: DatabaseSession, user: AuthenticatedUser
):
    from ...models import Recipient
    ...
    logger.info("Recipient updated", recipient_id=row.id, updated_by=user.get("username"))
    return RecipientSchema.from_orm(row)


@router.delete("/{recipient_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_recipient(recipient_id: int, db: DatabaseSession, user: AuthenticatedUser):
    from ...models import Recipient
    ...
    logger.info("Recipient deleted", recipient_id=recipient_id, deleted_by=user.get("username"))
    return None
