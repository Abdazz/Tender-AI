...
Date limite: {item2.get('deadline', item2.get('deadline_at', 'N/A'))}"""
        ...
        # Parse response
        is_duplicate = (
            "true" in response_text.lower() or "doublon" in response_text.lower()
        )
        ...
        if "confidence" in response_text.lower():
            try:
                ...
                conf_match = re.search(
                    r"(?:confidence|confiance)[:\s]+([0-9.]+)", response_text.lower()
                )
                if conf_match:
        ...
        for item in state.relevant_items:
            ...
            # Method 3: Hash + Similarity (default)
                ...
                if content_hash and content_hash in seen_hashes:
                    ...
                    item_ref = (
                        item.get("reference") or item.get("ref_no") or ""
                    ).strip()
                    for unique_item in unique_items:
                        # Reference number match is a strong duplicate signal
                        unique_ref = (
                            unique_item.get("reference")
            ...
            # Method 5: Hybrid (Hash + Similarity + LLM for edge cases)
                    ...
                    for unique_item in unique_items:
                        ...
                        # Moderate similarity - use LLM to decide
                            ...
                            if is_dup_llm and conf > 0.7:
                                is_duplicate = True
                                duplicate_reason = (
                                    f"llm_moderate_sim_{similarity}%_conf_{conf:.2f}"
                                )
                                item["duplicate_of_id"] = unique_item.get(
                                    "id", "unknown"
                                )
                                logger.debug(
                                    "LLM confirmed duplicate with moderate similarity",
                                    item_id=item.get("id"),
                ...
                logger.warning(
                    f"Unknown deduplication method: {method}, using hash_similarity"
                )
