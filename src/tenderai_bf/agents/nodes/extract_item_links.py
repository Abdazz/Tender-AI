...
def extract_links_from_html(content: str, base_url: str, patterns: dict) -> list[str]:
    ...
    try:
        ...
        link_selector = patterns.get("item_link_selector") or patterns.get(
            "item_selector"
        )
        ...
        for selector in selectors_to_try:
            try:
                ...
                if links:
                    logger.debug(
                        "Found links with selector", selector=selector, count=len(links)
                    )
                    break
        ...
        logger.error("HTML link extraction failed", error=str(e), exc_info=True)
        return []


def extract_links_from_pdf_list(
    content: str, base_url: str, patterns: dict
) -> list[str]:
...
def is_likely_tender_link(url: str, link_text: str = "") -> bool:
    ...
    has_include = any(
        pattern in url_lower or pattern in text_lower for pattern in include_patterns
    )

    # Check for exclude patterns
    has_exclude = any(
        pattern in url_lower or pattern in text_lower for pattern in exclude_patterns
    )
...
def extract_item_links_node(state) -> dict:
    ...
    try:
        ...
        seen_pdf_parsers = (
            set()
        )  # Track (url, parser_type) tuples for PDFs to allow different parsers
        source_link_counts = {}
            ...
            try:
                ...
                if parser_type == "pdf_quotidien":
                    ...
                    try:
                        ...
                        logger.error(f"Failed to parse {parser_type} results: {e}")
                        links = []
                ...
                state.add_error(
                    "extract_item_links",
                    f"Failed to extract links from {source_name}: {e!s}",
                    source_name=source_name,
                )
        ...
        logger.info(
            ...
            sources_processed=len(
                [item for item in state.items_raw if item["status"] == "success"]
            ),
