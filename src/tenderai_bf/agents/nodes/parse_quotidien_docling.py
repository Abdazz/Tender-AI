...
def parse_quotidien_with_docling(
    pdf_content: bytes, source_url: str, quotidien_title: str
) -> list[dict]:
    """
    ...
    try:
        ...
        pipeline_options = PipelineOptions(
            do_table_structure=True, do_ocr=True, ocr_options=ocr_options
        )

        converter = DocumentConverter(
            artifacts_path="/app/cache/huggingface", pipeline_options=pipeline_options
        )
        ...
        try:
            ...
            )  # Fixed: render_as_markdown() instead of document.export_to_markdown()
...
def extract_tenders_from_text(
    text: str, source_url: str, quotidien_title: str
) -> list[dict]:
    """
    ...
    fournitures_match = re.search(
        r"Fournitures et Services courants", text, re.IGNORECASE
    )
