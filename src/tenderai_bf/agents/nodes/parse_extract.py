...
def split_quotidien_into_tenders(text: str) -> list[dict]:
    ...
    if fournitures_start == -1:
        logger.error(
            "Could not find 'Fournitures et Services courants' section in quotidien"
        )
        return []
    ...
    for match in re.finditer(ref_pattern, avis_section):
        references.append(
            {"ref": match.group(0).strip(), "pos": match.start(), "end": match.end()}
        )
    ...
    for i, ref_info in enumerate(references):
        ...
        for line in reversed(pre_lines):
            ...
            if (
                ...
                and 25 < len(line_stripped) < 150
                and not any(
                    skip in line_stripped
                    for skip in [
                        "AVIS",
                        "SOURCE",
                        "FINANCEMENT",
        ...
        if len(tender_content) > 200:
            tenders.append(
                {"entity": entity, "ref_no": ref_info["ref"], "content": tender_content}
            )
...
def parse_quotidien_pdf(
    pdf_content: bytes, quotidien_url: str, quotidien_title: str, run_id: str
) -> list[dict]:
    ...
    try:
        ...
        return parse_quotidien_pdf_fallback(
            pdf_content, quotidien_url, quotidien_title, run_id
        )


def parse_quotidien_pdf_fallback(
    pdf_content: bytes, quotidien_url: str, quotidien_title: str, run_id: str
) -> list[dict]:
    ...
    Returns:
        List of parsed tender dictionaries
    """
    logger.info("Using fallback pdfminer parser", title=quotidien_title, run_id=run_id)

    try:
        # Extract text from PDF using pdfminer
        text = extract_pdf_text_from_bytes(pdf_content)

        logger.info("PDF text extracted", text_length=len(text), run_id=run_id)
        ...
        logger.info(f"Found {len(references)} reference numbers", run_id=run_id)
...
def extract_tender_info(section_text: str, index: int) -> dict:
    ...
    if re.search(
        r"travaux|construction|r[ée]habilitation|am[ée]nagement",
        section_text,
        re.IGNORECASE,
    ):
        info["category"] = "Travaux"
    elif re.search(
        r"fourniture|acquisition|achat|[ée]quipement|licence",
        section_text,
        re.IGNORECASE,
    ):
        info["category"] = "Fournitures"
    elif re.search(
        r"service|prestation|consultation|[ée]tude", section_text, re.IGNORECASE
    ):
    ...
    location_match = re.search(
        r"(?:Ouagadougou|Bobo-Dioulasso|Koudougou|Banfora|Tenkodogo|Fada N.Gourma|Burkina Faso)",
        section_text,
        re.IGNORECASE,
    )
    if location_match:
...
def parse_extract_node(state) -> dict:
    ...
    try:
                ...
                try:
                    ...
                    rag_tenders = parse_pdf_with_rag(
                        ...
                        metadata={
                            ...
                            "title": item.get(
                                "title", ""
                            ),  # Include quotidien title for date extraction
                        },
                        use_llm=True,
        ...
        state.update_stats(
            items_parsed=len(parsed_items), parse_time_seconds=time.time() - start_time
        )
