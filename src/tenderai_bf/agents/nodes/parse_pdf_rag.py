...
def extract_text_from_pdf(pdf_path: str) -> str:
    ...
    try:
        ...
        if text:
            logger.info(
                "PDF text extracted successfully with Docling",
                pdf_path=pdf_path,
            ...
            raise ValueError(
                "Unable to extract text from Docling result - no suitable method found"
            )
        ...
        try:
            ...
            logger.error(
                "Fallback extraction failed", error=str(fallback_error), exc_info=True
            )
            raise
...
def index_pdf_in_vector_store(
    ...
    try:
        ...
        logger.info("PDF split into chunks", pdf_path=pdf_path, chunk_count=len(chunks))
        ...
        doc_ids = vector_store.add_documents(
            source_name=source_name, documents=chunks, metadatas=metadatas
        )
        ...
        logger.error(
            "Failed to index PDF", pdf_path=pdf_path, error=str(e), exc_info=True
        )
        raise
...
def extract_tenders_with_llm(
    ...
    try:
        ...
        extraction = extract_tenders_structured(
            context=context, source_name=source_name, max_retries=2
        )
...
def parse_pdf_with_rag(
    ...
    try:
        ...
        if use_direct_extraction:
            ...
            if use_llm and chunks:
                for i, chunk in enumerate(chunks, 1):
                    ...
                    try:
                        extraction = extract_tenders_structured(
                            context=chunk, source_name=source_name, max_retries=2
                        )
                        ...
                        for j, tender in enumerate(extraction.tenders, 1):
                            ...
                            tender_dict["content_hash"] = hashlib.sha256(
                                hash_content.encode()
                            ).hexdigest()
                ...
                logger.error(
                    "LLM not available or no chunks extracted", source=source_name
                )
                if temp_file:
                    import os
        ...
        logger.info(
            ...
            total_documents_indexed=len(results.get("ids", [[]])[0])
            if results.get("ids")
            else 0,
        )
        ...
        for idx, tender in enumerate(tenders):
            if isinstance(tender, dict):
                ...
                tender["content_hash"] = hashlib.sha256(
                    hash_content.encode()
                ).hexdigest()
