...
class VectorStore:
    ...
    def get_or_create_collection(
        self, source_name: str, metadata: dict[str, Any] | None = None
    ) -> Any:
        """Get or create a collection for a source (Chroma 1.3.4+)."""
        collection_name = (
            f"{self.collection_prefix}_{source_name.replace(' ', '_').lower()}"
        )
        ...
        collection = self.client.get_or_create_collection(
            ...
            metadata=metadata
            or {"source": source_name, "created": datetime.now().isoformat()},
    ...
    def query_similar(
        ...
        try:
            ...
            logger.debug(
                ...
                source=source_name,
                documents_count=len(results.get("documents", [[]])[0])
            ...
            logger.error(
                "Query failed", error=str(e), source=source_name, exc_info=True
            )
            raise

    def delete_source_documents(
        self, source_name: str, where: dict[str, Any] | None = None
    ) -> int:
        """Delete documents from a source."""
        collection = self.get_or_create_collection(source_name)

        logger.info(
            "Deleting documents", source=source_name, collection=collection.name
        )
        ...
        try:
            ...
            logger.error(
                "Delete failed", error=str(e), source=source_name, exc_info=True
            )
            raise
    ...
    def reset_collection(self, source_name: str) -> None:
        ...
        logger.error(
            "Resetting collection", source=source_name, collection=collection_name
        )
        ...
        try:
            ...
            logger.error(
                "Failed to delete collection", error=str(e), collection=collection_name
            )
...
def get_vector_store() -> VectorStore:
    ...
    if _vector_store is None:
        ...
        try:
            ...
            logger.error(
                "Failed to initialize vector store", error=str(e), exc_info=True
            )
            raise
