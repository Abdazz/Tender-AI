...
def build_report(data: dict[str, Any]) -> bytes | None:
...
def _add_title_page(document: Document, data: dict[str, Any]) -> None:
    ...
    brand_desc_run = brand_desc.add_run(
        "Système autonome de veille des appels d'offres"
    )
...
def _add_executive_summary(document: Document, data: dict[str, Any]) -> None:
    ...
    if errors:
        ...
        error_para.add_run(
            f"⚠️ {len(errors)} erreur(s) rencontrée(s) durant l'exécution. Voir la section Annexes pour plus de détails."
        )
...
def _add_notices_section(document: Document, data: dict[str, Any]) -> None:
    ...
    if not notices:
        ...
        no_notices.add_run(
            "Aucun avis d'appel d'offres pertinent trouvé pour cette période."
        )
        no_notices.runs[0].font.italic = True
        return

    # Count appel_offres
    appel_offres_count = sum(
        1 for n in notices if n.get("type", "appel_offres") == "appel_offres"
    )
    country_name = data.get("country_name", "Burkina Faso") or "Burkina Faso"
...
def _add_other_notices_section(document: Document, data: dict[str, Any]) -> None:
    ...
    if not notices_by_type:
        ...
        no_notices = document.add_paragraph()
        no_notices.add_run(
            "Aucun autre avis (rectificatif, prorogation, communiqué, annulation) trouvé pour cette période."
        )
        no_notices.runs[0].font.italic = True
        document.add_page_break()
        return
...
def _add_notice_card(document: Document, notice: dict[str, Any], index: int) -> None:
    ...
    notice_title = notice.get(
        "tender_object", notice.get("title", "Titre non disponible")
    )
    ...
    summary_text = notice.get(
        "description", notice.get("summary_fr", "Résumé non disponible")
    )
...
def _add_appendices(document: Document, data: dict[str, Any]) -> None:
    ...
    if not errors:
        ...
        for i, error in enumerate(errors, 1):
            error_para = document.add_paragraph()
            error_para.add_run(
                f"{i}. [{error.get('step', 'unknown')}] {error.get('error', 'Unknown error')}"
            )
